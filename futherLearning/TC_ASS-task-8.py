...
# 4. In Flask, request.args and request.form are used to access data from incoming HTTP requests

# - request.args. It is used to access data sent in the query string of a URL. It is used with the GET requests for tasks like search queries, pagination and filtering data. example include
# search_category = request.args.get('q')

# - request.form. It is used to access form data submitted in the body of an HTTP request. It is generated by HTML forms and uses POST, PUT OR DELETE methods. It is commonly used in POST request for login, registration and creating resources. Example
# password = request.form.get('password')


# 5. SQL and NoSQL databases are two differenct types of databases with each one of them having its own major characteristics. Their majo differences include:
