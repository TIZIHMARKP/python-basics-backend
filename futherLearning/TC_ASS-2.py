...
# === Solution 6 ===
"""
Infinite loops are loops which never stop executing because its termination condition is never met and it brings about some the following risk
- Program freezes or becomes stuck and would not respond to any users input
- The loop consumes much of CPU core which makes your computer slow, hot and draining battery
- Forced termination(Cntrl + c) of the program by the user can lead to a risk of potentially losing unsaved work
Infinite loops can be avoided by:
- Ensuring every loop condition eventually evaluates to false
- Include a break condition with a safety limit for any condition where it may apply
- Use timeouts for user inputs
"""
